...
TASK_CONFIG_KEYS = {
    ...
    'IMAGE_PROMPT_GENERATION_MODEL_EXTERNAL': 'task.image_prompt.model.external',
...
async def generate_image_prompt(request: Request, form_data: dict, user=Depends(get_verified_user)):
    ...
    image_prompt_model_external = (
        await Config.get('task.image_prompt.model.external')
        or await Config.get('task.model.external')
    )
    task_model_id = get_task_model_id(
        model_id,
        await Config.get('task.model.default'),
