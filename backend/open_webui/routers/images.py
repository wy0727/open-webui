...
IMAGE_CONFIG_KEYS = {
    ...
    'IMAGES_MAGIC_API_BASE_URL': 'image_generation.magic.api_base_url',
    'IMAGES_MAGIC_API_KEY': 'image_generation.magic.api_key',
    ...
    'IMAGES_EDIT_MAGIC_API_BASE_URL': 'images.edit.magic.api_base_url',
    'IMAGES_EDIT_MAGIC_API_KEY': 'images.edit.magic.api_key',
...
async def get_image_model(request):
    ...
    elif image_config.IMAGE_GENERATION_ENGINE == 'magic':
        return image_config.IMAGE_GENERATION_MODEL if image_config.IMAGE_GENERATION_MODEL else 'dall-e-3'
...
async def verify_url(request: Request, user=Depends(get_admin_user)):
    ...
    if image_config.IMAGE_GENERATION_ENGINE == 'automatic1111':
        ...
        base_url = image_config.IMAGES_MAGIC_API_BASE_URL
        if not base_url:
            await Config.upsert({'image_generation.enable': False})
            raise HTTPException(
                status_code=400,
...
async def image_generations(
    ...
    try:
        if image_config.IMAGE_GENERATION_ENGINE == 'openai':
            ...
            if image_config.IMAGES_MAGIC_API_KEY:
                headers["Authorization"] = f"Bearer {image_config.IMAGES_MAGIC_API_KEY}"
...
async def image_edits(
    ...
    try:
        if image_config.IMAGE_EDIT_ENGINE == 'openai':
        ...
        elif image_config.IMAGE_EDIT_ENGINE == 'magic':
            base_url = image_config.IMAGES_EDIT_MAGIC_API_BASE_URL
            if not base_url:
                raise HTTPException(
                    status_code=400,
                    detail=ERROR_MESSAGES.INCORRECT_FORMAT(
            ...
            headers = {}
            if image_config.IMAGES_EDIT_MAGIC_API_KEY:
                headers["Authorization"] = f"Bearer {image_config.IMAGES_EDIT_MAGIC_API_KEY}"
            ...
            form_payload = {
                ...
                "size": size if size else image_config.IMAGE_EDIT_SIZE,
