	...
	>
		...
		<div class="flex-1 min-h-0 overflow-y-auto scrollbar-hover pr-1.5">
			...
			<AdminSettingSection title={$i18n.t('Generation')}>
				...
				<AdminSettingField label={$i18n.t('Image Prompt Generation Model (External)')}>
					<SettingsSelect
						bind:value={taskConfig.IMAGE_PROMPT_GENERATION_MODEL_EXTERNAL}
						className="w-full"
						placeholder={$i18n.t('Select a model')}
						on:change={() => {
							taskConfig.IMAGE_PROMPT_GENERATION_MODEL_EXTERNAL = normalizeModelSelection(
								taskConfig.IMAGE_PROMPT_GENERATION_MODEL_EXTERNAL
							);
						}}
					>
